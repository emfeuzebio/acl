        ...
        $(document).ready(function () {
            ...
            */
            $("#datatables tbody").delegate('tr td .btnEditar', 'click', function (e) {
                ...
                // SOLUÇÃO PARA usar awiat precisa chamar uma função separada

                // SOLUÇÃO DIRETA sem AWAIT
                fetch('https://apieventos.voluntary.com.br/api/organizacao/' + id, {
                    method: 'GET',
                    headers: { 
                        'Content-Type': 'application/json', 
                        'Authorization': 'Bearer 19|HUm7ao29lyhtDjo9BGKwL8DbbbNl22i79cIvURn07c5bfa08'
                ...
                .then(data => {
                    ...
                    // prepara o Form Modal
                    $('#modalLabel').html('Editar Organização');    // seta o título do Modal
                    $(".invalid-feedback").text('').hide();         // limpa todas as mensagens de erro anteriores
                    $('#editarModal #btnInserir').hide();           // esconde o botão Inserir
                    $('#editarModal #btnSalvar').show();            // mostra o botão Salvar
                    $('#form-group-id').show();                     // mostra a div do ID do registro
                    $('#editarModal').modal('show');                // show the modal

                    // carrega os dados para o o Form Modal
                    $('#id').val(data.id);
            ...
            */
            $("#datatables tbody").delegate('tr td .btnExcluir', 'click', function (e) {
                ...
                $('#confirmaExcluirModal').find('.modal-footer #confirm').on('click', function (e) {
                    ...
                    // DELETE via FETCH usado usado para excluir um recurso existente sem await
                    fetch('https://apieventos.voluntary.com.br/api/organizacao/' + id, {
                        method: 'DELETE',
                        headers: { 
                            'Content-Type': 'application/json', 
                    ...
                    .then(response => {
                        ...
                        if (! response.ok) {
                            ...
                            // Lança um erro para ser capturado no catch com o status e a mensagem retornada pela API JSON, caso tenha
                            return response.json().then(errorData => {

                                if (arrStatusErros.includes(response.status)) {
                                    $('#msgOperacaoExcluir').text('[' + response.status + '] - ' + errorData.message).show();
                                }
                                throw { success: response.status, message: errorData.message || 'Sem dados' };
                            });                            
                    ...
                    .catch(error => {
                        // error contém a mensagem lançada no throw new Error() acima.
                        console.log('Erro: ' + error.success + ' - ' + error.message);
                    }); 
                }); 
            }); 
            ...
            */
            $('#btnInserir').on("click", function (e) {
                ...
                formDados.forEach((value, key) => {
                    ...
                    // POST via FETCH usado usado para excluir um recurso existente sem await
                    fetch('https://apieventos.voluntary.com.br/api/organizacao', {
                        method: 'POST',
                        headers: { 
                            'Content-Type': 'application/json',
                            'Authorization': 'Bearer 19|HUm7ao29lyhtDjo9BGKwL8DbbbNl22i79cIvURn07c5bfa08'
                    ...
                    .then(response => {
                        ...
                        // sucesso: a API retorna um JSON com os dados do novo registro dentro de response.data
                        $("#alert .alert-content").text('Registro ID ' + data.id + ' inserido com sucesso.');
                        $('#alert').removeClass().addClass('alert alert-success').show().delay(5000).fadeOut(1000);
                        $('#editarModal').modal('hide');
                        $('#datatables').DataTable().ajax.reload(null, false);
                    }).catch(error => {
                        // error contém a mensagem lançada no throw new Error() acima.
                        console.log('Catch Erro: ' + error.success + ' - ' + error.message);
